...
export class AuthInterceptor implements HttpInterceptor {
  ...
  handle401Error(req: HttpRequest<any>, next: HttpHandler) {
    return this.authService.refresh().pipe(
      switchMap((res: DefaultResponseType | LoginResponseType) => {
        let error = '';


        if ((res as DefaultResponseType).error !== undefined) {
          error = (res as DefaultResponseType).message;
        }

        const refreshResult = res as LoginResponseType;
